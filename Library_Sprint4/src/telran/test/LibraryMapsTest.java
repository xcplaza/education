...
class LibraryMapsTest {
	...
	private static final int PICK_PERIOD1 = 30;
	...
	static Book book1 = new Book(ISBN1, TITLE1, AUTHOR1, AMOUNT1, PICK_PERIOD1);
	...
	private static final LocalDate PICK_DATE1 = LocalDate.of(YEAR1, 12, 7);
	private static final String TITLE2 = "title2";
	private static final String AUTHOR2 = "author2";
	private static final int AMOUNT2 = 1;
	private static final int PICK_PERIOD2 = 5;
	static Book book2 = new Book(ISBN2, TITLE2, AUTHOR2, AMOUNT2, PICK_PERIOD2);
	...
	public void removeBookTest() {
		assertNull(library.removeBook(ISBN1 + 100));// no book item
		// removing book that in use
		RemovedBookData rbd = library.removeBook(ISBN1);
		System.out.println(rbd);
		Book book = rbd.getBook();
		assertEquals(ISBN1, book.getIsbn());
		assertEquals(-1, book.getAmount());
		assertEquals(1, book.getAmountInUse());
		assertNull(rbd.getRecords());
		/*********************************/
		// removing book that hasn't been ever picked
		assertEquals(BooksReturnCode.OK, library.addBookItem(book2));
		rbd = library.removeBook(ISBN2);
		assertNull(library.getBookItem(ISBN2));
		assertEquals(ISBN2, rbd.getBook().getIsbn());
		assertTrue(rbd.getRecords().isEmpty());
		/******************************/
		// return book that has been removed
		returnRemovedBook();
	}

	private void returnRemovedBook() {
		RemovedBookData rbd;
		rbd = library.returnBook(ISBN1, READER_ID1, PICK_DATE1.plusDays(PICK_PERIOD1));
		List<PickRecord> removedRecords = rbd.getRecords();
		assertEquals(1, removedRecords.size());
		PickRecord expected = new PickRecord(ISBN1, READER_ID1, PICK_DATE1);
		expected.setDelayDays(0);
		expected.setReturnDate(PICK_DATE1.plusDays(PICK_PERIOD1));
		assertEquals(expected, removedRecords.get(0));
		assertTrue(library.getPickedRecordsAtDates(PICK_DATE1, PICK_DATE1.plusDays(1)).isEmpty());
	}

	@Test
	public void returnBookTest() {
		RemovedBookData rbd = library.returnBook(ISBN1, READER_ID1, PICK_DATE1.plusDays(PICK_PERIOD1));
		assertEquals(book1, rbd.getBook());
		assertNull(rbd.getRecords());
		library.removeBook(ISBN1);
		assertNull(library.returnBook(ISBN1, READER_ID1, PICK_DATE1.plusDays(PICK_PERIOD1)));
	...
	public void removeAuthorTest() {
		...
		library.removeAuthor(AUTHOR1);		
		returnRemovedBook();// test everything regarding removed book and removed author
	}
