...
//	Sprint 4
	...
	public List<Book> getMostPopularBooks(LocalDate fromDate, LocalDate toDate, int fromAge, int toAge) {
		List<PickRecord> rRecords = getPickedRecordsAtDates(fromDate, toDate);
		Map<Long, Long> mapPopularBooks = rRecords.stream().filter(r -> isProperAge(r, fromAge, toAge))
				.collect(Collectors.groupingBy(r -> getBookItem(r.getIsbn()).getTitle(), Collectors.counting()));
...
//		return res;
	...
	private boolean isProperAge(PickRecord r, int fromAge, int toAge) {
		LocalDate pDate = p.getPickDate();
		Reader reader = getReader(p.getReaderId());
		int readerAge = (int) ChronoUnit.YEARS.between(pDate, reader.getBirthData());
		int fromAgeInt = (int) ChronoUnit.YEARS.between(pDate, fromAge);
		int toAgeInt = (int) ChronoUnit.YEARS.between(pDate, toAge);
		return readerAge >= fromAgeInt && readerAge < toAgeInt;
//		LocalDate rentDate = r.getPickDate();
//		Reader reader = getReader(r.getReaderId());
//		int readerAge = rentDate.getYear() - reader.getBirthData();
//		return readerAge >= fromAge && readerAge < toAge;
	}

	@Override
	public List<String> getMostPopularAuthors() {
		long max = authorBooks.values().stream().mapToLong(x -> x.size()).max().getAsLong();
		List<String> res = new ArrayList<>();
		Map<String, Long> popularAuthor = 
		bookRecords.forEach((k, v) -> {
			if (v.size() == max)
				res.add(k.get);
		});
		return res;
...
//		return res;
	...
	@Override
	public List<Reader> getMostPopularReaders(LocalDate fromDate, LocalDate toDate) {
		List<PickRecord> rRecords = getPickedRecordsAtDates(fromDate, toDate);
		Map<Reader, Long> pReader = rRecords.stream()
				.collect(Collectors.groupingBy(r -> getReader(r.getReaderId()), Collectors.counting()));
		long max = (long) pReader.values().stream().mapToDouble(x -> x).max().getAsDouble();
