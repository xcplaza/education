...
//		this method causes issue in removeAuthor but it isn't needed
	...
	{
		List<PickRecord> possibleDelaying = 
				getPickedRecordsAtDates(dateFrom, currentDate
						.minusDays(minPickPeriod));
		Map<Reader, Integer> readersTotalDelay = possibleDelaying.stream()
				...
				.map(r -> getReaderDelay(r, currentDate))
				.filter(rd -> rd.getDelay() > 0)
		        .collect(Collectors.groupingBy(rd -> rd.getReader(), 
		        		Collectors.summingInt(rd -> rd.getDelay())));
	...
	{
		int delay = (int) (ChronoUnit.DAYS.between(record.getPickDate(), 
				currentDate)
	...
	private List<ReaderDelay> getList(Map<Reader, Integer> readers)
	{
		...
		res.sort((rd1, rd2) -> Integer.compare(rd2.getDelay(), 
				rd1.getDelay()));
		return res;
	}
	...
	{
		Map<Reader, Integer> readersTotalDelay = records.values().stream()
				.flatMap(List::stream)
		        .filter(r -> r.getReturnDate() != null && r.getDelayDays() > 0)
		        	.map(r -> new ReaderDelay(getReader(r.getReaderId()), 
		        			r.getDelayDays()))
		        	.collect(Collectors.groupingBy(rd -> rd.getReader(), 
		        			Collectors.summingInt(rd -> rd.getDelay())));
	...
	public List<String> getMostPopularAuthors()
	{
		Map<String, Long> authorCount = records.values().stream().flatMap(List::stream)
		        .collect(Collectors.groupingBy(r -> getBookItem(r.getIsbn()).getAuthor(), 
		        		Collectors.counting()));
