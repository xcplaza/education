...
public class AddModelItem extends RentCompanyItem {
	...
	public void perform() {
		...
		if (modelName == null)
			return;
		Integer gasTank = inOut.inputInteger(
				String.format("Enter gas tank volume from range [%d - %d]", MIN_TANK, MAX_TANK), MIN_TANK, MAX_TANK);
		if (gasTank == null)
			return;
		...
		if (country == null)
			return;
		Integer priceDay = inOut.inputInteger(
				String.format("Enter price day from range [%d - %d]", MIN_PRICE, MAX_PRICE), MIN_PRICE, MAX_PRICE);
		if (priceDay == null)
			return;
